...
export function applyHierarchicalLayout(snapshot: GraphSnapshot): GraphSnapshot {
  const nodes = cloneNodes(snapshot.nodes);
  if (nodes.length === 0) return { ...snapshot, nodes };

  positionAgentLayer(nodes, snapshot.edges);
  positionSatellites(nodes, snapshot.edges);
