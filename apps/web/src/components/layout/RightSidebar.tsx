...
import type { RuntimeExplanationProjection, RuntimeInterruptState, RuntimeSummary } from '@agentlens/protocol';
...
export function RightSidebar({ missionId, onBranchChange, runtimeExplanation = null, runtimeSummary = null }: RightSidebarProps) {
  ...
  const { projection: nodeProjection } = useNodeProjection({ missionId, agentId: selectedAgentId, branchId: currentBranchId ?? undefined, sequenceNum: frameSequenceNum, events: events as never, runtimeSummary });
  ...
  const pendingInterrupts = useMemo(() => Object.values(currentState?.interrupts ?? {}).filter((interrupt) => {
    if (interrupt.decision_state === 'recorded') return false;
    if (interrupt.status !== 'pending' && interrupt.request_lifecycle && interrupt.request_lifecycle !== 'pending') return false;
    if (interrupt.governance_available === false && (interrupt.framework === 'langgraph' || interrupt.framework === 'ms_agent_framework')) return false;
    if (interrupt.actionability && interrupt.actionability !== 'actionable' && interrupt.supported_decision_types?.length) return false;
    return interrupt.status === 'pending' || interrupt.actionability === 'actionable';
  }), [currentState]);
  const observedOnlyInterrupts = useMemo(() => Object.values(currentState?.interrupts ?? {}).filter((interrupt) => interrupt.status === 'pending' && ['observed_only', 'unavailable', 'identity_conflict'].includes(interrupt.actionability ?? '')), [currentState]);
      ...
      <div className="flex-1 overflow-hidden relative">
        <AnimatePresence mode="wait">
          {activeTab === 'inspect' ? <motion.div key="inspect" initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} className="h-full overflow-y-auto p-4 space-y-4">
            <div className="rounded-lg border border-[rgba(103,232,249,0.12)] bg-[rgba(103,232,249,0.04)] px-3 py-2 text-[10px] text-[#8f95b2]">branch {currentBranchId ?? 'unavailable'} | seq #{runtimeExplanation?.as_of_sequence_num ?? frameSequenceNum ?? 'unavailable'} | {runtimeExplanation?.projection_version ?? 'runtime_explanation.v1'}</div>
          ...
          </motion.div> : <GovernPanel pendingInterrupts={pendingInterrupts} observedOnlyInterrupts={observedOnlyInterrupts} recentDecisions={recentDecisions} decisionComment={decisionComment} setDecisionComment={setDecisionComment} decisionError={decisionError} isSubmittingDecision={isSubmittingDecision} onDecision={handleDecision} />}
        </AnimatePresence>
      </div>
    </aside>
  );
...
function GovernPanel({ pendingInterrupts, observedOnlyInterrupts, recentDecisions, decisionComment, setDecisionComment, decisionError, isSubmittingDecision, onDecision }: { pendingInterrupts: RuntimeInterruptState[]; observedOnlyInterrupts: RuntimeInterruptState[]; recentDecisions: RuntimeInterruptState[]; decisionComment: string; setDecisionComment: (value: string) => void; decisionError: string | null; isSubmittingDecision: boolean; onDecision: (id: string, decision: 'approve' | 'reject' | 'revise' | 'resume') => Promise<void> }) {
  return <motion.div key="govern" initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} className="h-full overflow-y-auto p-4 space-y-4">
    <div className="space-y-3"><span className="text-[11px] font-bold uppercase tracking-wider text-[#fbbf24] flex items-center gap-1.5"><AlertTriangle size={13} />Human Review Queue</span><textarea value={decisionComment} onChange={(event) => setDecisionComment(event.target.value)} placeholder="Reviewer notes / justification comments..." className="w-full h-20 resize-none px-3 py-2 text-xs rounded-xl bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.08)] text-[#e8eaf0]" />{decisionError && <div className="p-2 text-[11px] rounded bg-[#7f1d1d]/30 text-[#fecaca] flex items-center gap-2"><XCircle size={12} /><p>{decisionError}</p></div>}
      {pendingInterrupts.length ? pendingInterrupts.map((interrupt) => <InterruptCard key={interrupt.interrupt_id} interrupt={interrupt} isSubmitting={isSubmittingDecision} onDecision={onDecision} />) : <div className="flex flex-col items-center py-8 text-center bg-[rgba(255,255,255,0.01)] rounded-xl border border-dashed border-[rgba(255,255,255,0.04)]"><CheckCircle2 size={24} className="text-[#34d399]/40 mb-2" /><p className="text-xs text-[#9498b0] font-medium">No pending interrupts</p>{observedOnlyInterrupts.length > 0 && <p className="text-[10px] text-[#5d6180] mt-2">{observedOnlyInterrupts.length} observed interaction(s) are not actionable.</p>}</div>}
    </div>
...
function InterruptCard({ interrupt, isSubmitting, onDecision }: { interrupt: RuntimeInterruptState; isSubmitting: boolean; onDecision: (id: string, decision: 'approve' | 'reject' | 'revise' | 'resume') => Promise<void> }) {
  ...
  const supported = frameworkGovernance ? interrupt.supported_decision_types ?? [] : ['approve', 'reject', 'revise', 'resume'];
  const actionable = interrupt.governance_available !== false && interrupt.decision_state !== 'recorded' && (interrupt.actionability === 'actionable' || (!frameworkGovernance && !interrupt.actionability)) && supported.length > 0;
